...
def p2p_connect(local_address, local_port, send_file_path, recv_folder_path,server_address,server_port):
    ...
    if not os.path.exists(recv_folder_path):
        os.mkdir(recv_folder_path)  # 若为windows 只有mkdir
...
if __name__ == '__main__':
    ...
    file_path="text.txt"  # 待传输文件
    folder_path="recv"  # 接收文件文件夹
    remote_address="192.168.9.15"  # 服务器地址
    remote_port=31000  # 服务器端口
    p2p_connect(local_address,local_port,file_path,folder_path,remote_address,31000)